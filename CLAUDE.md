...
- `BucketsUpdate{NeoForge,Fabric}` — entry point with `MOD_ID = "buckets_update"`
- `BaseBucketItem` — shared bucket logic (extends `BucketItem`, restricts to water, hooks `canUseFor` / `applyWear` / `buildResult`)
- `WoodenBucketItem`, `CopperBucketItem`, `WaxedCopperBucketItem` — concrete implementations
- `OxidationStage` — enum with `Codec` + `StreamCodec`
- `ModItems`, `ModDataComponents`, `ModCreativeTabs` — registries
- `BucketEvents` — right-click handler (wax + scrub) + iron bucket recipe override

**6 items per bucket family** (empty + filled pair):
- `wooden_bucket` / `wooden_water_bucket`
- `copper_bucket` / `copper_water_bucket`
- `waxed_copper_bucket` / `waxed_copper_water_bucket`

`craftRemainder` chains the empty version when filled is consumed in a recipe, mimicking vanilla.

## Resource override pattern

The vanilla iron bucket recipe is replaced by ours (chain-on-top pattern):
...
- `item.buckets_update.<id>` — item display names
- `item.buckets_update.bucket.water_only` — overlay msg when trying to fill from non-water source
- `item.buckets_update.copper_bucket.too_rusty` — overlay msg when oxidized copper refuses water

## Textures

18 textures total under `assets/buckets_update/textures/item/`, generated by **recoloring vanilla `bucket.png` and `water_bucket.png`** with per-stage palettes. Script lives in commit history; pattern is:
1. Open vanilla bucket templates from `neoforge/build/neoForm/.../assets/minecraft/textures/item/`
2. Detect grey pixels (R≈G≈B) and remap their luminance to a 2-stop material palette
3. Preserve water blue pixels (R<G<B) as-is

Palette dict in head: wood (walnut→tan), copper unoxidized (burnt orange), exposed (warm tan), weathered (green-grey), oxidized (verdigris), and waxed_* (saturated/golden versions).
...
- **Adding a new bucket variant** (e.g., gold bucket): copy the copper pattern. New item pair, new recipe, new texture pair, optional new oxidation/wax mechanic. ~1 hour each loader.
